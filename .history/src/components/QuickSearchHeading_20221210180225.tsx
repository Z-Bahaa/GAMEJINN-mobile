...
const QuickSearchHeading = ({primaryText, ButtonOnPress}) => {
  ...
  return (
    <HStack  style={styles.container} dataSet={{ media: ids.container}} >
      ...
      <Button variant="unstyled" p={0} style={styles.button} dataSet={{ media: ids.button}} onPress={butt}>
        <HStack>
          <Text style={styles.buttonText} dataSet={{ media: ids.buttonText}} >view all</Text>
          <Icon as={MaterialIcons} name="arrow-forward-ios" size='14px' color={colors.textSecondary}  ml={.5}  />
        </HStack>
      </Button>
    </HStack>
  )
